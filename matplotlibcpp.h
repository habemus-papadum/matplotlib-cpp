...
// optional but recommended
...
#ifndef WITHOUT_NUMPY
        _import_array(); // initialize numpy C-API
#endif
        ...
        if (!s_python_function_show || !s_python_function_draw ||
            !s_python_function_pause || !s_python_function_figure ||
            !s_python_function_plot || !s_python_function_fill_between ||
...
bool errorbar(const std::vector<NumericX> &x, const std::vector<NumericY> &y,
              const std::vector<NumericX> &yerr, const std::string &s = "") {
    assert(x.size() == y.size());
...
#if __cplusplus > 199711L || _MSC_VER > 1800
// C++11-exclusive content starts here (variadic plot() and initializer list
// support)
...
template <typename T> struct is_callable_impl<true, T> {
    ...
    template <typename U>
    static std::true_type test(...); // use a variadic function to make sure (1)
                                     // it accepts everything and (2) its always
                                     // the worst match
...
bool named_plot(const std::string &name, const std::vector<double> &x,
